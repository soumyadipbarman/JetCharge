...
class Jetcharge_skimNANO(Module):
        ...
        def __init__(self,EventLimit=100):                          #local
				...
				for irecojet,recojet in enumerate(recojets):
					...
					if (irecojet == 0):
						...
						for i,c in enumerate(constituents):
							...
							self.h_goodrecojetchgpt1.Fill(constituents[i].Perp(), self.totalWeight)
								#self.h_goodrecojetchgpt1.Fill(recocands4v.Perp(), self.totalWeight)
			                                        #self.h_goodrecojetchgeta1.Fill(constituents.p4().Eta(), self.totalWeight)
                        			                        #self.h_goodrecojetchgphi1.Fill(constituents.p4().Phi(), self.totalWeight)			
								print '111'
